package com.ominfo.staff_original.interfaces;

public interface SharedPrefKey {
    String DEVICE_ID = "d_id";
    String IS_ATTACHED_FINGER_PRINT = "is_finger_print_attacked";
    String IS_NOTIFICATION_ENABLED = "is_notification_disable";
    String USER_IMAGE = "user_image";
    String CHOOSE_RECEIPIENT = "choosereceipient";
    String APP_SESSION = "appsession";
    String SET_DEFAULT_ACCOUNT = "set_default_account";
    String STANDING_ORDER_TYPE = "standing_order_type";
    String AFRICA_DELIVERY_MODE = "africa_delivery_mode";
    String MIGRATION_FLOW_STATE = "MIGRATION_FLOW_STATE";
    String MIGRATION_FLOW_DATA = "MIGRATION_FLOW_DATA";
    String MIGRATION_FLOW_COUNTRY = "MIGRATION_FLOW_COUNTRY";
    String IS_FIREBAE_TOKEN_UPDATED = "FIREBAE_TOKEN_UPDATED";
    String SHA256_CLIENTKEYHASH = "SHA256_CLIENTKEYHASH";
    String SHA256_PUBLICKEYHASH = "SHA256_PUBLICKEYHASH";
    String RANDOM_STRING = "random_string";
    String USER_PROFILE_IMAGE = "USER_PROFILE_IMAGE";
    String PIN = "pin";
    String IS_LOCK_BALANCE = "is_lock_balance";
    String ACCESS_TOKEN = "acces_token";
    String IS_LOGGED_IN = "is_logged_in";
    String KATA_CHITTI_DATE = "KATA_CHITTI_DATE";
    String IS_INVALID_PASSWORD = "IS_INVALID_PASSWORD";
    String FILE_PATH = "FILE_PATH";
    String IS_COMPLETE_REGISTRATION = "IS_COMPLETE_REGISTRATION";

}
